...
async def fban_p(message: Message):
    ...
    if ("-r" or "-s") in message.flags:
        if not FBAN_LOG_CHANNEL:
            ...
            return
        channel_ = await kannax.get_chat(int(FBAN_LOG_CHANNEL))
    ...
    log_fwd = await kannax.forward_messages(
        int(PROOF_CHANNEL),
        from_chat_id=from_,
        message_ids=proof,
    )
    ...
    async for data in FED_LIST.find():
        ...
        try:
            if chat_id != from_ and "-s" not in message.flags:
                fwd = await kannax.forward_messages(
                    chat_id=chat_id,
                    from_chat_id=from_,
                    message_ids=proof,
                )
    ...
    await kannax.send_message(PROOF_CHANNEL, log_msg_, disable_web_page_preview=True)
...
async def mass_fban(user, reason):
    async for data in FED_LIST.find():
        chat_id = int(data["chat_id"])
        try:
            await kannax.send_message(chat_id, f"/fban {user} {reason}")
