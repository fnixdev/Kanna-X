...
if kannax.has_bot:
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"^(secret|troll)_(.*)"))
    async def alive_callback(_, c_q: CallbackQuery):
        secret_path = "kannax/xcache/secret.json"
        ...
        try:
            sender = await kannax.get_users(int(view_data.get("sender")))
        except (BadRequest, IndexError):
            sender = await kannax.get_me()
