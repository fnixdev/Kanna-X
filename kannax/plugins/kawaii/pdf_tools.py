# ported from ultroid userbot to KannaX by @Kakashi_HTK/@ashwinstr
...
async def img_pdf(message: Message):
    ...
    if not input_:
        ...
        await kannax.download_media(reply, path)
        ...
        for one in lst:
            ...
            await kannax.send_photo(message.chat.id, lst_one)
...
async def text_pdf(message: Message):
    ...
    edt = await message.edit("Processing...")
    if not input_:
        file = await kannax.download_media(reply)
    ...
    if media.endswith((".jpg", ".jpeg", ".png", ".webp")):
        ...
        while True:
            ...
            if os.path.exists(abc):
                ...
                break
        await kannax.download_media(reply, abc)
        await message.edit(
            f"Done, now reply another image/pdf, if completed then use {Config.CMD_TRIGGER}pdf_send to merge and send all as pdf...",
        )
    else:
...
async def send_pdf(message: Message):
    ...
    merger.write(name_)
    await kannax.send_document(message.chat.id, name_, reply_to_message_id=reply)
    os.remove(name_)
