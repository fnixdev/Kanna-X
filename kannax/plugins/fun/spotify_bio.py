...
LOG_ = kannax.getLogger(__name__)
    ...
    async def spotify_bio_():
        ...
        while Config.SPOTIFY_MODE:
            ...
            try:
                ...
                full = await kannax.get_chat((await kannax.get_me()).id)
                bio = full.bio
                ...
                if to_insert:
                    ...
                    if new_bio:
                        ...
                        if not new_bio == bio:
                            try:
                                await kannax.update_profile(bio=new_bio)
                ...
                # not to_insert means no playback
                    ...
                    # back to the original one
                        ...
                        await kannax.update_profile(bio=SP_DATABASE.return_bio())
...
async def spotify_bio_toggle(message: Message):
    ...
    if Config.SPOTIFY_MODE:
        Config.SPOTIFY_MODE = False
        if USER_INITIAL_BIO:
            await kannax.update_profile(bio=USER_INITIAL_BIO["bio"])
        ...
        USER_INITIAL_BIO["bio"] = (
            await kannax.get_chat((await kannax.get_me()).id)
        ).bio or ""
...
@kannax.on_cmd("spnow", about={"header": "Now Playing Spotify Song"})
...
@kannax.on_cmd("spinfo", about={"header": "Get Info about Your Songs and Device"})
...
@kannax.on_cmd("spprofile", about={"header": "Get Your Spotify Account Info"})
...
@kannax.on_cmd("sprecents", about={"header": "Get Recently Played Spotify Songs"})
